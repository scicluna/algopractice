...
def generate_bc(url, separator):
    ...
    url_length = len(seperated_url)
    route = '/'

    if "https" in seperated_url[0] or "http" in seperated_url[0]:
        seperated_url = seperated_url[2:]
        url_length = len(seperated_url)

    if "index" in seperated_url[-1]:
        url_length = len(seperated_url)-1

    if len(seperated_url[-1]) == 0:
        print(seperated_url[-1])
        url_length = len(seperated_url)-1

    for i in range(url_length):
        name = name_change(seperated_url[i])# will need its own function to abbreviate in the case of long figures and to caps it.
    
        if i == 0 and i == url_length-1:
            final_string += f'<span class="active">HOME</span>'
        elif i == url_length-1:
            final_string += f'<span class="active">{name}</span>'
        elif i == 0:
            final_string += f'<a href="/">HOME</a>{separator}'
        else:
            route += f'{seperated_url[i]}/'
            final_string += f'<a href="{route}">{name}</a>{separator}'
...
def name_change(name):
    ...
    new_name = name.upper().replace('?', '#').replace('.', '#').split('#')
    banned_words = ["the","of","in","from","by","with","and", "or", "for", "to", "at", "a"]
    abbreviation = ""
    rootphrase = 0

    if new_name[0].lower() == "www":
        rootphrase = 1

    if len(new_name[rootphrase]) < 30:
        return new_name[rootphrase].replace('-', ' ')
    
    for word in new_name[rootphrase].split('-'):
        if word.lower() not in banned_words:
            abbreviation += word[0]
    return abbreviation
    

print(generate_bc("https://www.agcpartners.co.uk/index.html", " + "))